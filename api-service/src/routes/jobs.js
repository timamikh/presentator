...
router.post('/', authRequired, upload.array('files', 10), async (req, res) => {
  try {
    ...
    await query(
      `INSERT INTO presentator.jobs (id, user_id, prompt, status, file_paths, created_at, updated_at)
       VALUES ($1, $2, $3, 'pending', $4, now(), now())`,
      [jobId, req.user.id, prompt, JSON.stringify(filePaths)],
    );

    triggerWebhook(jobId, prompt, filePaths, req.user.id);
...
router.get('/:id', authRequired, async (req, res) => {
  try {
    const result = await query(
      `SELECT id, user_id, prompt, status, file_paths, slide_data,
...
router.patch('/internal/:id', internalAuth, async (req, res) => {
  try {
    const { status, slideData, resultPath, errorMessage } = req.body;
    const sets = [];
    const params = [];
    let idx = 1;
...
function triggerWebhook(jobId, prompt, filePaths, userId) {
